...
- Use a URL the browser can read. For a Windows browser, translate a WSL file path with `wslpath -w` and encode it as a file URI. If local-file navigation is blocked, serve only the sheet directory using Python's standard library: `python3 -m http.server <free-port> --bind 127.0.0.1 --directory <sheet-directory>`. Keep that process alive while collecting answers and use its loopback HTTP URL. Keep the HTML self-contained; serving is only a viewing aid. A remote CDP browser must be able to reach the URL from its own host.
