...
- `unavailable`: ast-grep was not on PATH. Continue source reading and record the concrete reason on the affected function-flow content.
- `failed`: the requested extraction could not be completed or verified. Continue source reading and record the reason. Nodes are empty so a failed batch cannot masquerade as complete evidence.

If the helper itself cannot run or write JSON, record that failure and continue source reading. Keep `evidence.json` alongside the session model. Use a distinct report directory to preserve unrelated artifacts. The shared dotfiles tool environment supplies ast-grep; modeling itself does not install tools or change the target project's dependencies.
