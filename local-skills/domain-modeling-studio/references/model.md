...
The generator writes new files only. Four outputs are `model.md` (a per-layer Mermaid diagram for each layer present, plus each node/edge's kind, `drillInto`, origin and evidence), `glossary.md`, `scenarios.feature`, `changes.md`. All four deliverables draw from every layer present in the model, not only the business flow. The artifacts must be regenerated after AI review of semantic changes; `needsReview` flags pre-review derived content. Mermaid is an output view, not a lossless editing format.
