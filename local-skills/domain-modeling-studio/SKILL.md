...
## Generate the architecture layer

Scope architecture nodes to only the module(s) the chosen business flow actually touches, plus their import/require dependency neighbors one hop out; never scan the whole repository ([ADR-0062](../../docs/adr/0062-scope-architecture-layer-to-neighborhood.md)). Run the neighborhood scanner — `node <skill-dir>/scripts/generate-architecture.mjs <repo-root> <revision> <seed-file...>` — passing the business-flow-touched files as seeds and the same pinned commit as `revision`; it resolves relative imports into `MODULE` nodes and everything else into `EXTERNAL` nodes, and never scans a resolved neighbor's own imports, so the result is bounded to seeds plus one hop by construction.

The scanner is a single-line text scan of import/require-like statements, not a type-aware resolver: it misses multi-line or grouped import blocks, and it cannot see a runtime-loaded resource that never appears as an import/require statement (e.g. a template file opened via `readFile`). When such a dependency matters to the flow, cite it as business-flow evidence instead of inventing an architecture edge for it. Every node and edge the scanner produces carries `origin: inference` (a lightweight text match, never proof of a runtime dependency); only a human confirming it upgrades that to `agreement` or `code`.

An architecture node's `drillInto` may only name a node inside its own function-flow layer graph, never a business-flow node directly. If this session has not generated a function-flow layer for that module yet, leave `drillInto` unset on its architecture nodes and record that omission as an `unresolved` entry rather than fabricating a target.
...
The standalone template and notices in `dist/` ship with the skill. `src/` is the editable source. `node scripts/build.mjs` uses the pinned package lock in a fresh temporary directory, leaving npm dependencies outside the chezmoi-deployed tree. Rebuild the template after source changes. Run `node --test tests/*.test.mjs` (model, architecture-generation and real-content coverage) and the browser workflows in `tests/browser.mjs` and `tests/browser-real-content.mjs` (their environment requirements are at the top of each). Verify offline `file:` loading, actual edits/undo/restore, clipboard failure and baseline conflicts before replacing the template. The `tests/browser.mjs` fixture is deliberately fictional; `tests/browser-real-content.mjs` instead drives the layer-selector drilldown against a model produced by actually running `generateArchitecture` and the function-flow procedure, so the two never drift apart in shape. Private pilot data belongs in the target task's scratch directory.
