...
既存の取得状態だけでフローを表せるなら、表示値は採用する操作の識別を使って binding から導出する。例えば Mediator に最新操作 ID を保持し、pending／result はその ID の binding 状態から読む。追加の排他や取消制約がある場合だけ、その制約を Mediator の状態として表す。Atom を使うこと自体は、フローの裁定が成立する保証ではない。
