...
- Determine the base ref and the current branch. Resolve the base ref from origin's actual default branch, never from the current branch's upstream tracking or `git status` wording (a worktree created off a stale or non-default ref would otherwise silently carry that ref through to the PR): `git ls-remote --symref origin HEAD | awk '$1 == "ref:" && $3 == "HEAD" { print $2; exit }'`, strip the `refs/heads/` prefix. Step 6's `gh pr create` re-runs this same resolution inline (like `TO_PR_SCRATCH_BASE`, this does not carry across separate shell invocations) rather than assuming a variable set here still exists there. If the branch is not pushed, that is handled in step 5.
...
3. Push the current topic branch if needed with `git-push-topic`. The wrapper accepts no arguments, rejects the default branch, and internally uses the fixed `origin HEAD` form. Then create the PR, resolving and passing `--base` explicitly in the same command block — never omit it and never let `gh pr create` infer a base from the branch's own tracking, which can silently point at whatever ref the topic branch happened to fork from:

   ```bash
   git-push-topic
   default_branch="$(git ls-remote --symref origin HEAD | awk '$1 == "ref:" && $3 == "HEAD" { print $2; exit }')"
   default_branch="${default_branch#refs/heads/}"
   gh pr create --title "<conventional title>" --base "$default_branch" --body-file <tmp>
   ```
