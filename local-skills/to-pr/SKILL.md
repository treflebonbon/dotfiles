...
- Determine the base ref and the current branch. Resolve the base ref from origin's actual default branch, never from the current branch's upstream tracking or `git status` wording (a worktree created off a stale or non-default ref would otherwise silently carry that ref through to the PR): `git ls-remote --symref origin HEAD | awk '$1 == "ref:" && $3 == "HEAD" { print $2; exit }'`, strip the `refs/heads/` prefix. Reuse this resolved value in step 6 rather than re-deriving it. If the branch is not pushed, that is handled in step 5.
