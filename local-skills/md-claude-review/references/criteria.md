...
| 過剰検証・スコープ逸脱 | "回答前に必ず自己検証する" / "サブエージェントに自分の出力を再確認させる" のように、モデル自身の出力を一般的に再チェックさせる指示（`npm test` 実行・release verifier・ブラウザ AC 検証等、§3 で維持すべきプロジェクト固有の検証コマンドとは異なる） | Opus 5 は指示なしで自己検証・自己修正するため、モデル自身の出力を一般的に再チェックさせる一文だけを削除する。§3 が維持対象とするプロジェクト固有の検証コマンド・手順は対象外で、削除しない | Prompting Claude Opus 5 — Task scope and over-verification / Self-correction |
