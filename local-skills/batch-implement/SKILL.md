...
4. **On apparent completion**, read the child's recent output (`herdr agent read <name>` or `herdr pane read`) to judge whether it actually finished the chain (every ticket committed) or stalled/failed partway. This is a judgment call, not a fixed string match — read enough of the transcript to be sure.
   - **Success**: prompt the same session once more, `herdr agent prompt <name> "/to-pr" --wait --until idle --until done --until blocked --timeout 900000` (15 minutes is a starting point; tune similarly), then read its output for the resulting PR (number/URL) — treating `timeout`/`blocked` the same as step 3 above. Handing this chain to a fresh session via `batch-implement` is itself the AFK-style completion authorization for this one `to-pr` call — the same authorization `to-pr` already documents for AFK operation, scoped here to chains that actually finished. It does not authorize anything `to-pr` itself doesn't authorize (no force-push, no direct default-branch push, no merge, no close/reopen).
     ...
     2. **The parent, only via the helper — and only when the helper's own close targets actually name it.** Re-run `local-skills/to-pr/scripts/reconcile-ticket-hierarchy.sh` (see `to-pr`'s own step 4) with a fresh JSON input: the same `linkedIssue` and `hierarchy` snapshot `to-pr` already fetched (re-fetch it the same way `to-pr` step 1 does if it's no longer in context — do not substitute this chain's own ready-for-agent-filtered ticket list, which is not the same exhaustive, unfiltered child set `to-pr`'s native-hierarchy fetch or Hierarchy Repair verifies), and `coveredIssues` expanded to include every chain ticket from step 1 above. `state: "確認済み"` alone is not enough to add `Fixes #<parent>` — the script also returns `確認済み` when the parent is already **closed** (in which case `closeTargets.parent` is empty, correctly). Only add it when `closeTargets.parent` actually contains the parent's number.
        ...
        This native-hierarchy path also has a blind spot worth checking before you trust it: `to-pr` only runs Hierarchy Repair when the linked issue has _no_ native parent (`local-skills/to-pr/SKILL.md` step 1.2-1.3) — a native-parent hierarchy is read straight from `subIssues` and never repaired. So if step 1's union (native ∪ body-declared) found a batch ticket that's a body-only-declared child of this parent (no native sub-issue edge) and it's still open and not part of this chain, the helper's snapshot doesn't know it exists and can say `確認済み` regardless. Before adding `Fixes #<parent>`, also check that no such ticket remains open and uncovered; if one does, don't add it — treat this the same as `未実施`, naming that ticket, even if the helper itself said `確認済み`.

     Update `## Parent Reconciliation` with step 2's helper `state` and `reason` verbatim (append the body-only-child override reason above when it applies), and a close-target list that always includes every chain ticket whose `Fixes` line you added in step 1, plus the parent only when both checks in step 2 passed. If the helper can't run, rejects its input, or returns `未実施`, fall back the same way `to-pr` itself does: record the returned (or command-failure) reason, omit the parent `Fixes`, and keep the linked issue's ordinary `Fixes` plus the step 1 chain-ticket `Fixes` you already added. Then update the PR with `gh pr edit <pr> --body-file <updated-body>`.
