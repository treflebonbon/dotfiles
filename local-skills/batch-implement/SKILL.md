...
- For any ticket you'll designate as a multi-ticket chain's link target (step 3 always uses the chain's first, most-blocking ticket for this), also check it has a **reconciliation-capable hierarchy**: either a native parent whose body _also_ declares a matching `## Parent` (`to-pr` requires both sides present to cross-check — a native parent with no body declaration counts as a failed cross-check for `to-pr`, not a pass), or no native parent but a single valid body `## Parent` that Hierarchy Repair can complete. A ticket that's merely part of the batch (not a chain's link target) doesn't need this — only the one `to-pr` will resolve a hierarchy from. If the check fails, don't implement that ticket as part of a multi-ticket chain: report the whole chain as blocked pending a human fix to that ticket's hierarchy declaration, rather than implementing it and having step 3's reconciliation silently fail to close its siblings. A size-1 chain is unaffected — there's no sibling closure to guarantee.
...
4. **On apparent completion**, read the child's recent output (`herdr agent read <name>` or `herdr pane read`) to judge whether it actually finished the chain (every ticket committed) or stalled/failed partway. This is a judgment call, not a fixed string match — read enough of the transcript to be sure.
   - **Success**: prompt the same session once more, `herdr agent prompt <name> "/to-pr" --wait --until idle --until done --until blocked --timeout 900000` (15 minutes is a starting point; tune similarly), then read its output for the resulting PR (number/URL) — treating `timeout`/`blocked` the same as step 3 above. Handing this chain to a fresh session via `batch-implement` is itself the AFK-style completion authorization for this one `to-pr` call — the same authorization `to-pr` already documents for AFK operation, scoped here to chains that actually finished. It does not authorize anything `to-pr` itself doesn't authorize (no force-push, no direct default-branch push, no merge, no close/reopen).
     ...
     `to-pr` computes Parent Reconciliation from its one linked issue's perspective, so this chain's _other_ tickets look like uncovered siblings to it — even though this PR actually implements and covers them too. Fix the PR's closing keywords in two independent parts — do not expect `reconcile-ticket-hierarchy.sh`'s own `fixes` output to already cover this (see why below):

     1. **This chain's own siblings, unconditionally.** For every other ticket in this chain, add its Acceptance Criteria to the PR body's `## Contract` and one row per AC to `## Verification Matrix` — exactly what `to-pr` step 1-2 would do for its own linked issue. This _is_ `to-pr`'s definition of Ticket Coverage; a bare `Fixes` line without it does not establish coverage. Once documented this way, give it its own `Fixes #<n>` line directly — you already know it's genuinely covered by this PR (chain membership itself established that in step 2). Do **not** get this from re-running `to-pr`'s reconciliation helper (see step 2 below for where and how): its uncovered-child branch (the `ordinary(...)` result) returns a closing keyword for only the single linked issue whenever _any_ direct child of the parent is still open and uncovered — including one from a different, not-yet-implemented chain outside this run — regardless of what you pass as `coveredIssues`. That is a correct guard against closing tickets nobody's verified in the general case, but it has no way to know this chain's own tickets _are_ verified, so relying on its `fixes` here would silently drop them.
     2. **The parent, only via the helper — and only when the helper's own close targets actually name it.** Re-run `to-pr`'s own reconciliation helper (see `to-pr`'s own step 4) with a fresh JSON input — `<skill-directory>/../to-pr/scripts/reconcile-ticket-hierarchy.sh`, the sibling `to-pr` skill directory next to this one (both are always deployed together; a repo-relative path like `local-skills/to-pr/...` would resolve inside whatever repo this skill is running in, not this dotfiles source tree, and not exist there): the same `linkedIssue` and `hierarchy` snapshot `to-pr` already fetched (re-fetch it the same way `to-pr` step 1 does if it's no longer in context — do not substitute this chain's own ready-for-agent-filtered ticket list, which is not the same exhaustive, unfiltered child set `to-pr`'s native-hierarchy fetch or Hierarchy Repair verifies), and `coveredIssues` expanded to include every chain ticket from step 1 above. `state: "確認済み"` alone is not enough to add `Fixes #<parent>` — the script also returns `確認済み` when the parent is already **closed** (in which case `closeTargets.parent` is empty, correctly). Only add it when `closeTargets.parent` actually contains the parent's number.

        This native-hierarchy path also has a blind spot worth checking before you trust it: `to-pr` only runs Hierarchy Repair when the linked issue has _no_ native parent (`local-skills/to-pr/SKILL.md` step 1.2-1.3) — a native-parent hierarchy is read straight from `subIssues` and never repaired. So a body-only-declared child of this parent (no native sub-issue edge) that's still open is invisible to the helper's snapshot, and it can say `確認済み` regardless. Don't limit this check to step 1's own union — that search is filtered to `ready-for-agent`, so an open body-only child without that label wouldn't be in it either, recreating the same blind spot one level down. Before adding `Fixes #<parent>`, run an unfiltered scan the way `to-pr`'s own Hierarchy Repair does: every open issue (any label, or none) whose body declares `## Parent` pointing at this parent. If any such ticket isn't already closed or covered by this PR, override the helper's result: **set `state` to `未実施`** (not just its reason) and name that ticket, regardless of what the helper itself said.

     Update `## Parent Reconciliation` with the (possibly overridden) `state` and `reason` from step 2, and a close-target list that always includes every chain ticket whose `Fixes` line you added in step 1, plus the parent only when step 2's final `state` is `確認済み` and its `closeTargets.parent` names it. If the helper can't run, rejects its input, or returns `未実施`, fall back the same way `to-pr` itself does: record the returned (or command-failure) reason, omit the parent `Fixes`, and keep the linked issue's ordinary `Fixes` plus the step 1 chain-ticket `Fixes` you already added. Then update the PR with `gh pr edit <pr> --body-file <updated-body>`.
