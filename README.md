...
| シェル環境       | starship, zoxide, atuin, eza, bat, fzf, direnv, bash-completion, blesh |
| 検索             | ripgrep, fd, jq                                                        |
...
- **入力補助**: ble.sh（`blesh` パッケージ）による構文ハイライト・autosuggestion・リッチな Tab 補完メニュー。`--attach=none` で source しファイル末尾で `ble-attach`（[ADR-0018](docs/adr/0018-add-blesh-to-bash.md)）
- **ツール連携**: ble.sh, starship, atuin, fzf, zoxide, eza, direnv（init 順序: ble.sh → bash-preexec → fzf(ble-import) → atuin → starship → ble-attach）
- **履歴**: atuin（`Ctrl-R`）に一本化。ble.sh の autosuggestion も atuin の履歴に基づく
