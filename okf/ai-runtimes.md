...
- **AskUserQuestion がアイドルでも既定で自動継続しなくなった**（2.1.200）— `CLAUDE_AFK_TIMEOUT_MS: "60000"` を `settings.json.tmpl` に追加し、60秒アイドルで自動継続（recommended オプションを採用）へオプトイン。background agent / `teammateMode: auto` で無人実行が滞留しないようにするため。
...
- 経緯・判断根拠は [decisions/2026-07-04-advisor-tool-default-enable](decisions/2026-07-04-advisor-tool-default-enable.md) を参照。
