...
_User-invoked_（明示起動のみ、orchestration 層。チェーン: `grill-with-docs → to-prd → to-issues → triage → to-pr`）:
...
実装フェーズに user-invoked skill は無い: `triage` で `ready-for-agent` にした issue を agent に渡すと model-invoked 層が自動発火する。**非導入**: `implement` / `resolving-merge-conflicts`（上流 README 非掲載の unlisted skill。`implement` は tdd/code-review 発火で冗長な5行の糊）、`grilling`（fail-soft でモデルが inline 対話に代替でき、[decisions/2026-07-02-mattpocock-over-superpowers](decisions/2026-07-02-mattpocock-over-superpowers.md) が同系 `grill-me` を軽量化のため除去済み）。
