...
| mizchi/skills/empirical-prompt-tuning（floating、パス変更） | upstream `mizchi/skills` が `meta/` サブディレクトリを廃止しフラット構成へ移行したため、旧パス `meta/empirical-prompt-tuning` を指定した `apm install` が `Subdirectory not found` で失敗 | `apm.yml` の依存パスを `mizchi/skills/empirical-prompt-tuning` へ追従。パッケージ全体の `content_hash` は `sha256:20e7fba9...` → `sha256:26042612...` へ変化した（`README.md` の hash が `sha256:c7db7d27...` → `sha256:5fc8e777...` へ変化。差分は install 例の `apm install mizchi/skills/meta/empirical-prompt-tuning` 等3箇所を新パスへ書き換えただけで、実際のスキル本体である `SKILL.md`/`SKILL-ja.md` の hash は不変） |

`apm.yml` のパス修正後、隔離 cwd（Git remote なし）へコピーして `apm install --https`（apm 0.31.0）を実行した。生成 lock の SHA-256 `baa97ced11b444fe9c51da7fbd3b5648f8eecd5aec46b66f6d9e10cf7c686300` は同一環境の `apm install --frozen --https` 前後で不変、`apm audit --ci` は10/10全通過した。生成 lock を diff すると、変更対象は mizchi のパスと `README.md` 追従に伴う `content_hash`（実体は install 例の書き換えのみ、`SKILL.md`/`SKILL-ja.md` は不変）、`orchestration` / `computer-use` の resolved_commit と content hash（実体差分）、`shadcn` / `find-skills` の revision-only な `resolved_commit` 更新のみで、他13依存（`orca-cli` の exact pin `de0a91b9` を含む）の pin・content hash は無変化だった。両 target（`.agents/skills/` / `.claude/skills/`）で19依存の discovery を確認した。
