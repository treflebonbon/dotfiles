...
既存割当には `managed-chrome-owner relocate --role playwright --workspace <physical-root> --identity <exact-identity>` を追加した。登録 lock 内で、正確な既存 identity・所有権記録なし・Dashboard 記録なし・旧 profile / port に Chrome なしを確認してから、その identity の port pair だけを更新する。profile と他 identity は保持する。残存予約、稼働 Chrome、Dashboard 記録、照会失敗は拒否する。再割当は wrapper と同じ runtime lock も取得し、競合時は待たずに拒否して lock 順序の逆転による待ち合わせを避ける。wrapper は runtime lock 取得後に割当を再照合し、待機中に変更されていれば操作前に再試行を求める。
...
## PR #311 の Dashboard 復旧経路のレビュー対応

Chrome 停止・owner 回収後に Dashboard だけが残る場合、旧実装の relocate はその listener の割当まで変更し、後続の `show --kill` を妨げた。Dashboard の記録が残る間は割当を変更せず、所有 session で先に Dashboard を終了する契約にした。runtime lock の競合と、wrapper が割当取得後に待機する間の再割当も保護する。

所有権22件と既存 wrapper68件の計90件が成功。追加の wrapper2件でも、Dashboard が残る状態の再割当拒否→元の `show --kill` 成功→再割当成功、および runtime lock 取得前に割当が変わった場合の操作拒否を確認した。Dashboard 記録各種と runtime lock 競合を扱う所有権の追加回帰も単独再実行で成功。ShellCheck・lint・Nix build 成功。実ブラウザを新たに起動せず、既存の wrapper lifecycle fixture で検証した。
