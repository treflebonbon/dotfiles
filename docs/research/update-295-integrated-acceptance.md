...
2026-09-10。親[#283](https://github.com/treflebonbon/dotfiles/issues/283)、子#286〜#295を同じvalidated worktree/branchで扱う。全対象の採否とsource側の受入検証を完了した。最終full Batsは658実行PASS・20skip・1環境失敗（exit1）。実装と統合文書の2軸レビューはStandards規約違反0/smell0、Spec0だった。live配備は受入・merge後に残す。
...
| 実6言語template gate | 確認済み | `merge-templates-result.json` / `merge-templates.log`。main取込み後の6独立repoで新規lock生成・3system評価・devShell/with-env・言語周辺CLI・実raw入口/ダミーテスト/commit/再起動まで6/6 PASS、skip0 |
| 最終full Bats | 実行済み・環境失敗1件 | `run-merge-regression-retry.py` → `merge-regression-retry.log` / `merge-regression-retry-result.json`。main取込み後の679件完走、658実行PASS・20skip・WSL interop失敗1件、exit1、842.94秒。6lock不変、終了後のChrome ownerはnull。関連Dogfood14件は12PASS/2環境失敗、通常復旧後の影響3件は3/3 PASS。`merge-dogfood-scoped-result.json` / `merge-dogfood-recovery-result.json` |
| 最終2軸review | 確認済み | 実装 `6412429...2d9aa85` と統合文書 `2d9aa85...a3c2e62` の各reviewはStandards規約違反0/smell0、Spec0。最終結果を反映した11文書差分も両軸で追加確認し、指摘0だった。main取込み `90fcb7a...3bbd70d` も両軸0 |

上の相対ログ名は原則`tmp/update-283/`に置く。ログはローカル一時証跡であり、PRの読者は各採用記録から版・結果・限界を確認できる。

main取込み前の初回full Batsは676件を完走しexit1、668 TAP ok（うち19skip）・8失敗だった。annotation2件と既存GitHub/model/resolver/proxy socket6件が、二重の`nix develop`により深くなったTMPDIRでAF_UNIXパス上限に達した。Playwrightの同じ上限は旧CLIにも存在する。`final-failure-evidence/`と初回`final-regression-result.json`を保持し、同じannotation2件を短いTMPDIRで再実行して2/2 PASSを確認した。source・テスト期待値は変更せず、再fullでは最内側のshellで短い専用TMPDIRを明示し、全件が失敗0で完走した。初回を成功扱いしない。
...
PR #296 の公開時にmainの3コミットとの競合を検出した。固定したmain `9e1ab06b4b0103a70db1a5fbfb5cc29938c1b524`（#282・#284・#285）を `3bbd70df6d8643558cc901e3139cc08badd39307` でtopicへmergeした。競合はtemplate helperのimportだけで、更新側のre/shutilとmain側のshlexをすべて保持した。Rust1.98.1、Gleam OTP29、周辺ツール検証と、mainの秘密なしraw検証・人間向け手順を両立させた。6テンプレートの共通inputはmainで受入済みの `63e47ffc471ff5e01f58a8a268ea553b0c9ab976` を保持する。

取込み前の統合結果は676件中657実行PASS・19skip、exit0（`final-regression-retry-result.json`、783.87秒）。取込み後の初回679件は678 TAP ok（20skip）・annotation未対応時のエラー文言1失敗、exit1だった。同じケースの単独再実行は1/1成功し、初回の詳細は通常cleanupで残っていないため原因は未確定とする。sourceやassertionは変更せず、失敗出力・一時証跡を保持して全679件を再実行した。2回目は#195が通過し、#191がPowerShell Resolve開始前のWSL `UtilAcceptVsock:273 accept4 failed 110` で失敗した（`merge-failure-evidence/191/report.md`）。2回目も678 TAP ok（20skip）・1失敗・exit1。全体を成功とせず、同じsourceの関連14件の再確認は12PASS/2環境失敗だった。unsupportedケースのCleanupにも同じWSL interop障害が発生し、所有権が安全側に保持されたため後続2件が拒否された。caller PIDの消滅とWindows Inspectによる自分のprofile/port/PID一致を確認し、既存CDP closeとmanaged-chrome-owner recoverで通常復旧した。force kill・所有権記録の削除は行わず、復旧後の影響3ケースは3/3 PASS、終了後ownerはnull。#292採用時の実14/14 PASSと今回の複数実行は別記録として保持する。最初の#195も同原因だったとは断定しない。上表はこの区別を保持する。過去の成功値をそのまま最新headの結果にしていない。型検査・Ruff check/format・template Nix format・通常commit hookも成功した。PRのOSV `scan-pr / osv-scan` は成功し、補助check `osv-scanner` はskipped。ブラウザー専用profileがGitHub未認証のため補助画像は手動添付待ちで、ローカル証跡bundleをPRへ記載した。
...
| **#283-AC16 — 回帰検証**: 変更に該当する既存品質ゲート、型検査、関連テスト、full Bats suite、必要な隔離chezmoi dry-runを実施する。失敗やskipを根拠なく成功として扱わず、必須の候補実体検証ができない変更は採用済みとしない。 | CLI/infra | 上記最終統合検証：full679件: 658PASS/20skip/1WSL環境失敗・exit1、関連14件12PASS/2環境失敗、復旧後の影響3件PASS・6lock不変、型検査・lint・隔離dry-run、2軸review | 確認済み | — |
...
| **#283-AC19 — 完了記録**: 全対象の採用・維持・保留と根拠、ACごとの検証結果、未確認のplatform・実機動作、後続の配備手順をまとめ、影響を受けた正本文書と整合させる。 | CLI/infra | 本記録の全76 AC原文照合、採否索引、最終検証結果と配備手順 | 確認済み | — |
...
| **#286-AC5**: frozen install、型検査、該当品質ゲート、full Bats suiteで採用する組み合わせを検証し、失敗・skip・実機未確認の範囲を記録する。 | CLI/infra | [#286記録](update-286-quality-tools.md)、root全6shell評価・WSL build、品質CLI、frozen npm、TS7型検査、Action lint/pin、最終full679件: 658PASS/20skip/1WSL環境失敗・exit1、関連14件12PASS/2環境失敗、復旧後の影響3件PASS・6lock不変・2軸review | 確認済み | — |
...
| **#287-AC5**: 既存のNix devShell・キャッシュ・起動契約テストとfull Bats suiteを実行し、採用する共通package集合を確定する。 | CLI/infra | [#287記録](update-287-user-environment.md)、user全6shell評価・WSL build、75公開CLI probe、startup/floor/cacheテスト、最終full679件: 658PASS/20skip/1WSL環境失敗・exit1、関連14件12PASS/2環境失敗、復旧後の影響3件PASS・6lock不変・2軸review | 確認済み | — |
...
| **#288-AC6**: 関連する実package出力・品質floor・起動テストとfull Bats suiteを通し、採用または維持するsnapshot、APM版、関連依存の根拠を記録する。 | CLI/infra | [#288記録](update-288-ai-tools.md)、AI全6shell評価・WSL build、25CLI/MCP probe、Herdr隔離起動終了、strict Codex設定検証、最終full679件: 658PASS/20skip/1WSL環境失敗・exit1、関連14件12PASS/2環境失敗、復旧後の影響3件PASS・6lock不変・2軸review | 確認済み | — |
...
| **#289-AC5**: full Bats suiteと必要な隔離dry-runを実行し、非互換候補は直前の採用済み組み合わせを保持する。 | CLI/infra | [#289記録](update-289-ordinary-apm-skills.md)、native APM install/frozen/audit、全17 selected tree、43/43 discovery・1,204配布hash・ownership、関連40 PASS/1 skip、最終full679件: 658PASS/20skip/1WSL環境失敗・exit1、関連14件12PASS/2環境失敗、復旧後の影響3件PASS・6lock不変・2軸review | 確認済み | — |
...
| **#290-AC5**: 隔離native lock・frozen no-rewrite・audit・discovery、関連hookテスト、full Bats suiteを通し、非Impeccable payloadの意図しないdriftを確認する。 | CLI/infra | [#290記録](update-290-impeccable.md)、native APM install/frozen/audit、56 selected files・3asset/hash、実engine14件＋managed failure1件、関連40 PASS/1 skip、最終full679件: 658PASS/20skip/1WSL環境失敗・exit1、関連14件12PASS/2環境失敗、復旧後の影響3件PASS・6lock不変・2軸review | 確認済み | — |
...
| **#291-AC3**: 候補を採用する場合は隔離materialization、frozen no-rewrite、audit、discovery、workflow契約、full Bats suite、隔離dry-runを順序どおり実行する。 | CLI/infra | [#291記録](update-291-matt-pocock.md)、whole skills tree/plugin blob同一、selected25/74files・両target148hash・198ledgerのread-only照合、最終full679件: 658PASS/20skip/1WSL環境失敗・exit1、関連14件12PASS/2環境失敗、復旧後の影響3件PASS・6lock不変・2軸review | 確認済み | 実体不変のため候補採用条件は不成立。ordered gate未実施 |
...
| **#292-AC5**: 関連品質ゲートとfull Bats suiteを実行し、共通基盤と組み合わせた採否を記録する。 | CLI/infra | [#292記録](update-292-playwright-dogfood.md)、全6shell評価・host build、native CLI/runner4件、WSL CLI/annotation/cleanup、実Dogfood14件＋契約107件、最終full679件: 658PASS/20skip/1WSL環境失敗・exit1、関連14件12PASS/2環境失敗、復旧後の影響3件PASS・6lock不変・2軸review | 確認済み | — |
...
| **#293-AC4**: 対応3 systemのNix評価、hostのbuild/起動、関連品質ゲートとfull Bats suiteを実施し、独立したCLIごとに採否を判断する。 | CLI/infra | [#293記録](update-293-fixed-clis.md)、全3system×2shell評価・source host build/start、19公開CLI probe＋flyline PTY、関連9件、最終full679件: 658PASS/20skip/1WSL環境失敗・exit1、関連14件12PASS/2環境失敗、復旧後の影響3件PASS・6lock不変・2軸review | 確認済み | — |
...
| **#295-AC1**: 全対象の現行・候補・採用版、維持・保留理由を一つの結果として統合し、担当漏れ・重複・食い違いを解消する。 | CLI/infra | [#295記録](update-295-integrated-acceptance.md)、上記の最終統合検証、76 AC照合、source dry-runと配備手順、最終full679件: 658PASS/20skip/1WSL環境失敗・exit1、関連14件12PASS/2環境失敗、復旧後の影響3件PASS・6lock不変・2軸review | 確認済み | — |
| **#295-AC2**: 19受入条件を個別証跡と統合sourceへ対応付け、packageの実際の版、APM payload、テンプレートの組み合わせを確認する。 | CLI/infra | [#295記録](update-295-integrated-acceptance.md)、上記の最終統合検証、76 AC照合、source dry-runと配備手順、最終full679件: 658PASS/20skip/1WSL環境失敗・exit1、関連14件12PASS/2環境失敗、復旧後の影響3件PASS・6lock不変・2軸review | 確認済み | — |
| **#295-AC3**: 組み合わせが変わった範囲で全体回帰、Nix/CLI、APM再現性、hook、テンプレートの既存検証を実行する。個別成功を統合成功と取り違えない。新しい変更の影響を受けない証跡は再利用する。 | CLI/infra | [#295記録](update-295-integrated-acceptance.md)、上記の最終統合検証、76 AC照合、source dry-runと配備手順、最終full679件: 658PASS/20skip/1WSL環境失敗・exit1、関連14件12PASS/2環境失敗、復旧後の影響3件PASS・6lock不変・2軸review | 確認済み | — |
| **#295-AC4**: 必要な隔離chezmoi dry-runを行い、3 system評価とhost実機動作、skip・未確認のplatformや機能を区別したVerification Matrixを作る。 | CLI/infra | [#295記録](update-295-integrated-acceptance.md)、上記の最終統合検証、76 AC照合、source dry-runと配備手順、最終full679件: 658PASS/20skip/1WSL環境失敗・exit1、関連14件12PASS/2環境失敗、復旧後の影響3件PASS・6lock不変・2軸review | 確認済み | — |
| **#295-AC5**: 影響する正本文書を最終採否と整合させ、受入・merge後のlive sourceからの配備と通常環境確認の手順・残作業を記録する。 | CLI/infra | [#295記録](update-295-integrated-acceptance.md)、上記の最終統合検証、76 AC照合、source dry-runと配備手順、最終full679件: 658PASS/20skip/1WSL環境失敗・exit1、関連14件12PASS/2環境失敗、復旧後の影響3件PASS・6lock不変・2軸review | 確認済み | — |
| **#295-AC6**: source側の完了とlive側の未実施/実施済みを区別して報告し、live非配備のままlive確認済みとは扱わない。 | CLI/infra | [#295記録](update-295-integrated-acceptance.md)、上記の最終統合検証、76 AC照合、source dry-runと配備手順、最終full679件: 658PASS/20skip/1WSL環境失敗・exit1、関連14件12PASS/2環境失敗、復旧後の影響3件PASS・6lock不変・2軸review | 確認済み | — |
...
- 最終fullの20skipはruntime mount1件、raw/isolated runtime・model/GitHub/MCP/dependencyのopt-in15件、実Herdr1件、実6言語template1件、with-env実Nix2件。templateの必須実体gateは別途6/6 PASS。残るopt-inは未実施であり、認証済みmodel/APIや実Herdrイベントの再検証を主張しない。main由来の人間検証の境界テストは今回のfullで実行PASS、6テンプレートの実raw入口も別gateでPASSした。
...
- 各固定asset、native APM、AI/補助CLIの個別probeは、対応するsource/lockがmain取込みでも変更されていないため再利用した。共通inputが変わった6言語gateと全体Batsは再実行した。
