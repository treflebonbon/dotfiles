...
- 抽出器contract: 12テスト成功。固定済み0.42.1を使った.ts/.tsx・Unicode範囲・構文エラー・CLI出力を含み、nativeテストのskipなし。
...
- PR #309追加レビュー対応: 親ノード探索を全件比較から包含スタックへ変更し、ソート後の探索をO(n)にした。同一範囲の除外・同率時の先頭ノード選択・兄弟・境界共有・ファイル切替を検証した。ast-grepのバージョン指摘は、共通devShellのlockをNix評価して0.42.1であることを再確認したため、package定義の追加変更は不要と判断した。
- 性能確認: 2,500関数・15,000選択ノードを固定済みast-grepで抽出した単発比較では、変更前32.407秒・変更後0.763秒。出力JSONは完全一致した。時間は当該環境の実測であり、保証値ではない。
