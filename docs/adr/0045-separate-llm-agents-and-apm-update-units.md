...
初回live APM frozen installではsource lockの書き戻しを検出した。Issue #187のordered gateはnon-Matt baselineを一時的にexact pinして検証するため、作業用lockの`resolved_ref`がunpinned dependencyにも残っていた。live manifestでnative layoutへ戻す際、Orca `computer-use` / `orchestration`は旧lockの`9c01e09ecc9d3c1203968ace9945d16edfb35dd2`からdefault-branch revision `026389a3bc03da03ca2d65295e805493712b0774`へ自然再解決された。両entryのcontent hashは`sha256:afe48be623c1f6190ade7dacc4c1d334d4150b503ed00b28dda23a499e5bdc30` / `sha256:b5c364878fb07d21a369f091f2e96b823da94308b15b39636f2585d8b5621b51`のままでpayload変更はない。このrevision-only driftを記録してnative lockをsourceへ戻し、live onchangeもlock生成と同じ`apm install --frozen --target claude,codex --https`へ統一した。regression testはmanifestのexact pin 5件だけに`resolved_ref`があることを固定する。

修正後はsource/live lockのSHA-256がともに`db4a0decf91aad223dbc4f33edad9ba57be399d5e64507b4d89b6d9b9a0740ee`となり、同じlive HOMEで二回目のfrozen installが書き戻しなし、`apm audit --ci`が10/10、scoped `chezmoi status --exclude=scripts`がemptyとなった。Modern Web Guidance / Remotion / ImpeccableとMatt Pocock 25 skillは`~/.agents/skills` / `~/.claude/skills`の両方で発見でき、Matt payloadには明示的Skill tool呼出し、frontier separator、setup未済時のuser pointerが含まれる。
...
| #183-4 selected subtree比較とexact pin境界        | infra             | Issue #185の18 dependency比較とfinal native lock。Modern Web Guidance / Remotionのみpayload変更pin、Orca `026389a3`はhash不変のrevision-only | 確認済み | —                                                                           |
| #183-5 native lock / frozen no-rewrite / audit    | CLI / infra       | live `apm install --frozen --target claude,codex --https`を二回実行し、二回目のlock SHA-256不変、source/live checksum一致、audit 10/10       | 確認済み | organization policy enforcementはHOMEにGit remoteがなくwarning付きskip      |
...
| #183-7 Impeccable 4.1.2とCodex native Stop        | API / infra       | exact pin `63b04e25`、managed pass-through wiring、materialized runtimeへの`turn_id`付きStop event                                           | 確認済み | 実Codex sessionからのhook発火は初回trust approval待ち                       |
...
| #183-9 Matt 25-skill managed set                  | CLI / infra       | exact pin `6654f6b6`、両target25/25、explicit invocation / separator / setup pointerをlive payloadで確認                                     | 確認済み | —                                                                           |
...
| #183-12 atomic adoption / fallback                | infra             | 四単位は全てgate成功で採用。exact pinを維持し、final lockのOrca revision-only移動はhash不変として記録。partial adoption / fallbackなし       | 確認済み | —                                                                           |
