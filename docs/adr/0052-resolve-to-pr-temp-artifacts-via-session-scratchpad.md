...
- **`dogfood` の裸 `mktemp -d` も同じ issue で直す**: 性質の異なる2種類の temp 生成（session をまたいで参照される成果物 vs 自己完結した検証実行の出力）を1つの変更に混ぜることになるため、スコープを `to-pr` に絞った。
