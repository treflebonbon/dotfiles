...
Accepted (2026-07-03)。ワークフローチェーンを単一チェーンとした部分は [ADR-0012](0012-branch-workflow-chain-by-scenario.md)（2026-07-06）でシナリオ別3分岐に amend 済み。to-pr が draft PR を開く部分は [ADR-0013](0013-to-pr-ready-for-review.md)（2026-07-07）で ready-for-review に amend 済み。`implement` 非導入の根拠は [ADR-0015](0015-add-tdd-commit-confirmation.md)（2026-07-07）で upstream issue の実証的な信頼性バグに基づき補強され、to-pr が前提とする commit 責務の空白も doc 層で埋められた。その後、上流 mattpocock/skills v1.1.0 の promoted set に合わせるため、`implement` / `resolving-merge-conflicts` 非導入判断と `to-prd` / `to-issues` 語彙は [ADR-0022](0022-align-mattpocock-v1-1-workflow.md)（2026-07-10）で置き換えられた。スクリーンショットを Git commit する方針は [ADR-0026](0026-attach-playwright-evidence-to-pr.md)（2026-07-22）で GitHub の PR 添付へ置き換えられた。
...
- 実装後のブラウザ AC 検証 + PR 作成は、軽量 chezmoi ローカル skill **`to-pr`** で補完する:
  - `local-skills/to-pr/` を SoT に、`run_onchange_after_deploy-local-skills.sh.tmpl` が `~/.agents/skills` / `~/.claude/skills` へ materialize（orphan-cleanup の `preserve_local_skills` で保護）。Codex は `~/.agents/skills` を直接読むため、`~/.codex/skills` へは重複配備しない
  - 変更が browser-observable なら `playwright-cli` で AC 検証、非 UI なら検証を skip して draft PR を開く
  - スクリーンショットの保存方針は [ADR-0026](0026-attach-playwright-evidence-to-pr.md) が置き換え、代表画像を GitHub の PR 添付として扱う
...
関連: [skill-harness](../../runtime/skill-harness.md) / [ADR-0002](0002-mattpocock-over-superpowers.md) / [ADR-0012](0012-branch-workflow-chain-by-scenario.md)（ワークフローチェーンを amend）/ [ADR-0026](0026-attach-playwright-evidence-to-pr.md)（画像保存方針を置き換え）
