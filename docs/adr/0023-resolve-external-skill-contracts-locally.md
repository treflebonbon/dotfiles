...
3. `harness-feedback` は system/developer 指示、project `AGENTS.md`、呼び出された skill の順に **実効契約**を解決してから逸脱を判定する。下位 skill の規則が上位指示で置き換えられた場合は finding にせず、必要なときだけ **Contract Warning** として報告する。
...
7. scope mismatch、上位指示による上書き、critical なレビュー欠落、同一 project transcript 不在の4ケースを normative examples として固定する。自動 eval の導入は本決定の範囲外とする。
