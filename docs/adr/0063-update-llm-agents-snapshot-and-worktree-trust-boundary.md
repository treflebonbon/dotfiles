...
- 通常 APM payload refresh を同じ回の別更新単位として採用する。APM binary は snapshot が供給する 0.31.0 を使う。実体差分は次の2件: `mizchi/skills` の upstream が `meta/` サブディレクトリを廃止しフラット構成へ移行したため、`apm.yml` の依存パスを `mizchi/skills/meta/empirical-prompt-tuning` から `mizchi/skills/empirical-prompt-tuning` へ追従した（選択済みスキルの content_hash `sha256:0a38d8...` は不変で、パス変更のみ）。`stablyai/orca` の floating `orchestration` / `computer-use` は selected content hash が変化し、それぞれの `SKILL.md` の routing 文言（Orca embedded browser・Playwright/CDP・programmatic path への誘導基準の明確化）が upstream で更新されていることを確認した。同じ floating dependency の `shadcn`・`orca-cli`・`find-skills` は resolved_commit のみ revision-only に前進し selected content hash は不変。他13依存（Impeccable skill・Matt Pocock full set・Effect-TS・modern-web-guidance・remotion-best-practices・herdr skill 等の exact pin 群）は不変のまま維持する。
