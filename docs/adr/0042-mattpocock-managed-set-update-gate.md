...
5. **skill discovery**: `.agents/skills` と `.claude/skills` の full set が一致し、各 target の `*/SKILL.md` を discovery probe で読み、frontmatter（`name` / `description`）が存在することを disposable runtime で確認する。Claude / Codex CLI の対話的 loader はこの gate から起動せず、既存の最高 seam である managed target layout と payload を検証する。
