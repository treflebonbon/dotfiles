...
- `batch-implement` — `to-tickets` が親 issue 配下に作った ready-for-agent ticket 群を、`## Blocked by`（本文テキスト + native blocking/tracked-by 関係）で連結した **Ticket Chain**（[CONTEXT.md](../CONTEXT.md)）単位に分解し、chain ごとに1 worktree・明示 `--base`（実際の default branch を `git ls-remote --symref origin HEAD` 相当で解決）・既存の `implement`（ticket を跨いで継続）→ chain 完了時のみ `to-pr` を一度、という既存 Builder-Evaluator 規約の割当てだけを担う薄いオーケストレーション層。`--base` を明示しない worktree 作成が前 chain の未マージ branch から分岐し main 宛てでない PR を生む問題への対応。並列 Workflow 化はしない（1 chain ずつ順次処理）。chain 内の失敗は `to-pr` を呼ばずその chain だけ未完了として報告し、他の chain の処理を続行する。連結成分・トポロジカル順序の算出は `scripts/compute-chains.sh`（純粋関数、`tests/batch-implement.bats` で検証）に切り出す
