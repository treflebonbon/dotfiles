...
**Matt Pocock managed full set の update gate**: candidate revision は [ADR-0042](../docs/adr/0042-mattpocock-managed-set-update-gate.md) の順序で評価する。隔離 runtime での lock generation → `apm install --frozen` → `apm audit --ci` → skill discovery → workflow contract tests → `chezmoi --source "$SOURCE_DIR" apply --dry-run` を一つの検証境界とし、全て通過した exact commit だけを採用する。`@latest` / `main` / native Claude plugin / universal installer は候補経路として拒否し、失敗時は accepted manifest / lock pair を保持して partial adoption を commit しない。
