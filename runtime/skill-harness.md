...
- **Frontier grilling**: `grilling` は依存関係が解決済みの全 decision を frontier round でまとめて提示し、各質問へ推奨を添え、複数質問の間を horizontal rule (`---`) で区切る。各 round の人間の回答を待ち、事実は環境探索で埋め、未回答の decision を推測して先へ進まない。`ui-grill-with-docs` も同じ frontier round を使い、visual comparison が必要な質問だけ disposable mockup を添える。
...
- **Worktree ownership**: Worktree Entry Point は workflow を validated task worktree から始める共通の入口契約とする。Orca は agent session の開始前に Orca native worktree を作成・選択し、Agent Picker から built-in agent を起動して permission mode も所有する。Orca native Codex は `codex-orca` / `codex-worktree` を使わない。local file の変更につながる engineering flow の開始時に current linked worktree を read-only に検証し、Orca primary checkout なら Orca CLI や raw Git を呼ばず、native worktree から新しい session を始めるよう案内して停止する。Orca の識別は runtime 自己認識に従い、`ORCA_*` environment の汎用判定は導入しない。非 Orca runtime は `to-worktree` を使い、Codex Desktop は native worktree、Claude Code は `EnterWorktree`、raw Codex CLI は caller `HEAD` から scoped approval 一回で作成した worktree を使う。raw CLI は作成後に `codex-worktree` を root とする fresh session のため停止し、parent change、fetch、別 path の same-topic worktree、permission bypass に触れない（[ADR-0044](../docs/adr/0044-runtime-owned-worktree-entry-and-codex-activation.md)、[ADR-0046](../docs/adr/0046-separate-orca-native-worktree-entry.md)）。
...
標準 hook の対象ルールは二層で、**両方を配線して初めて両 tier が届く**（[ADR-0029](../docs/adr/0029-impeccable-pin-advance-with-stop-hook.md)）。per-edit は immediate tier を編集箇所へ返し、その他の対象ルールは `Stop` へ先送りする。Stop はセッション中に触れた UI ファイルを再走査して fresh finding をまとめて返す（per-edit が既に出した分は dedupe。何も残っていなければ無言）。4.2.2 / engine 0.1.3 は上流既定の advisory 除外を使い、従来の deferred fixture `overused-font` は対象外となるため、`side-tab` で遅延検出を検証する。`Stop` は `stop_hook_active` を見て再入時は即座に抜ける。

Impeccable 4.1.2 は Stop scan 後に fresh finding だけでなく live finding 全体を cache へ同期する。immediate / deferred の両 tier を持つファイルでも初回 deep pass 後の次回の `Stop` は無言になり、新しい finding が現れるまで再報告しない。4.1.1 までの交互再報告は解消済みで、4.2.2 / engine 0.1.3でもこの動作を維持し、`tests/design-hook.bats` は実際の管理 commandからこの silent convergence を検証する。
...
- `to-worktree` — 非 Orca runtime の Worktree Entry Point。current linked worktree は Orca での誤起動を含めその場で検証し、Orca primary checkout は native worktree からの新しい session を案内して停止する。Orca では Agent Picker の built-in launch と Orca-owned permission mode を維持し、repository Runtime Adapter へ差し替えない。Codex Desktop / Claude Code は各 native Worktree Owner へ routing する。raw Codex CLI だけは caller `HEAD` から、同じ absolute physical top level を `git -C` と destination の両方に使う1 commandの成功を完了条件として `.worktrees/<topic>` を作り、`codex-worktree` による fresh session activation のため停止する。parent change は残し、fetch、別 path の same-topic worktree 再利用、manual Git fallback は行わない
...
raw adapter は検証後、`devshell-env` の repo 信頼を読み、対象 root の devShell を sandbox 起動前に準備する。登録情報の永続書込みは明示 CLI の `trust/untrust` だけが所有し、自動起動経路では登録しない。未登録・flake 不在・Nix／shellHook の失敗は理由付きでプロジェクト環境を加えず起動するが、起動引数・metadata 所属の異常は準備前後とも fail closed にする。起動先・標準 profile・Active Git Metadata Boundary は Nix に置換させない。準備時は `.envrc`・dotenv の値・秘密取得・永続プロジェクト環境キャッシュを追加せず、更新はセッション再起動で反映する（[ADR-0044 amendment](../docs/adr/0044-runtime-owned-worktree-entry-and-codex-activation.md#2026-09-09-amendment-信頼済み-devshell-の準備)、[利用方法](shell-environment.md#raw-codex-のプロジェクト開発環境)）。

#257 の通常 `with-env` は必ず Nix を準備し、raw の `with-env --prepared -- command` だけがこの準備済み環境を再利用して、現在の root `.env` を指定コマンドと子にだけ注入する。信頼済み準備が成功した raw 起動だけに root `.env` の read を許可し、追加 workspace root を無効化する。標準 profile の外側は最小ランタイム・Nix・Git 関連の読取り例外以外を拒否し、専用 `TMPDIR` と Active Git Metadata Boundary を維持する。詳細・名前付き app の正式入口・失敗時の再起動は [利用方法](shell-environment.md#指定コマンドへの-dotenv-注入) と [ADR-0044](../docs/adr/0044-runtime-owned-worktree-entry-and-codex-activation.md) に従う。
