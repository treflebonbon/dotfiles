...
同日の最終配備では chezmoi source を live HOME へ apply し、初回 smoke で Matt gate の一時 pin layout が source lock の unpinned dependency に `resolved_ref` を残していたことを検出した。live install はこれを native layout へ戻し、Orca `computer-use` / `orchestration` を revision-only の `026389a3bc03da03ca2d65295e805493712b0774` へ自然再解決したが、両 selected content hash は維持した。source lock をこの生成物へ戻し、live onchange も `--target claude,codex --https` を明示するよう統一した。最終 lock SHA-256 は source/live とも `db4a0decf91aad223dbc4f33edad9ba57be399d5e64507b4d89b6d9b9a0740ee`、二回目の frozen install は書き戻しなし、audit は10/10、Claude/Codexの通常payload・Impeccable・Matt 25 skill discovery、Design Hook 10/10とmanaged fail-open 3/3を通過した。Codex native hooks は配備済みだが新しいentryの実セッション実行には初回trust approvalが必要であり、permission bypassはせず未確認として残す（→ [ADR-0045](../docs/adr/0045-separate-llm-agents-and-apm-update-units.md)）。
