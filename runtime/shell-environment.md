...
通常の `chezmoi apply`、APM 配備、初回導入は `NIX_DEVSHELL_CACHE_REQUIRED=1` で更新し、必要な更新や前提の確認に失敗したら停止する。途中まで出力した Nix の非0終了、空・bash 構文不正の出力、キャッシュ置換の失敗は旧キャッシュを保持する。対話シェルの背景更新は引き続き起動を止めない。鮮度の判定は現時点では `.nix`／`flake.lock` の更新時刻を使い、入力内容や削除の検知、更新競合の制御は後続の実装対象（[ADR-0050](../docs/adr/0050-user-environment-cache-freshness.md)）。
