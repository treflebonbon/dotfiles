...
Requests to address pull request review feedback authorize a Review Round. For that workflow, use `gh-review-thread` rather than the GitHub plugin for thread-aware reads and writes. Group the selected threads into one Conventional Commit, verify the changes, publish with `git-push-topic`, then post a Japanese reply with the change and verification result to each addressed thread and resolve it. Explanation-only threads need no empty commit. Continue after per-thread failures; leave ambiguous, rejected, failed-verification, unpublished, or failed threads open and report why.
