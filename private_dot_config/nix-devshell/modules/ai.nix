...
let
  ...
  # 2.1.233: NT \\??\\ device path による UNC validation bypass / NTLM credential leak、skill / command
  #          argument の template marker 再展開、Linux sandbox idle CPU、MCP v2 reconnect を修正。
  #          2.1.232 の Cygwin-style symlink と input redirection の permission 変更は revert されたため、
  #          その2点を本 version の hardening 根拠にはしない。新しい model では TaskCreate/Get/Update/List と
  #          TodoWrite が既定で無効になるため、interactive workflow smoke を更新時の検証対象にする。
  # 更新: flake.nix の llm-agents revision を更新し、nix flake lock 後に flake.lock を re-addする。
  minClaudeCode = "2.1.233";
  minCodex = "0.147.0";
  ...
  claudeCode =
    let
      ...
      msg = ''
        ...
        確認できないまま pin が到達した 2.1.228 に加え、危険 flag の auto-approval 停止、PowerShell の
        permission bypass、nested repository の trust 継承、cross-session /tmp symlink と Linux protected-path の
        sandbox bypass、GitLab token redaction、project settings による sandbox.ripgrep override を修正する
        2.1.229-2.1.232、NT device path による UNC validation bypass / NTLM credential leak、skill / command
        argument の template marker 再展開、Linux sandbox idle CPU、MCP v2 reconnect を修正する 2.1.233 を根拠に、
        現在の ${minClaudeCode} を品質ベースラインとして固定しています
        （2.1.232 の Cygwin-style symlink と input redirection の permission 変更は 2.1.233 で revert され、
        新しい model では TaskCreate/Get/Update/List と TodoWrite が既定で無効になるため、interactive smoke を
        更新時の検証対象にしています）。
